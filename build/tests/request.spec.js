...
const promise_1 = require("../core/classes/promise");
describe('request.js', () => {
    it('should support promises', async (done) => {
        const url = 'https://jsonplaceholder.typicode.com/comments';
        try {
            const request = new promise_1.promise({ url });
    ...
    it('should support query parameters', async (done) => {
        const url = 'https://jsonplaceholder.typicode.com/comments';
        const parameters = { postId: 1 };
        try {
            const request = new promise_1.promise({ url, parameters });
            const response = await request.execute();
    ...
    it('should accept XML and return a JSON object', async (done) => {
        ...
        try {
            const request = new promise_1.promise({ method: 'POST', url, body, headers, contentType });
            const response = await request.execute();
            expect(response.statusCode).toEqual(200);
    ...
    xit('can chain multiple requests together', async (done) => {
        ...
        let url = 'http://petstore.swagger.io/v2/pet';
        let petId;
        try {
            // Create the pet 
            const createPet = new promise_1.promise({ method: 'POST', url, body });
            const pet = await createPet.execute();
            petId = pet.body.id;
            expect(petId).toBeDefined();
            // Find the pet
            url = `${url}/${petId}`;
            const findPet = new promise_1.promise({ url });
    ...
    it('should only return the body if resolveWithBodyOnly = true', async (done) => {
        ...
        const url = 'http://petstore.swagger.io/v2/pet';
        try {
            const request = new promise_1.promise({ method: 'POST', url, body, resolveWithBodyOnly: true });
            const response = await request.execute();
    ...
    it('should default to rejecting non 2xx status codes', async (done) => {
        ...
        try {
            const request = new promise_1.promise({ url, rejectNon2xx: false });
            const response = await request.execute();
    ...
    it('should support timeouts', async (done) => {
        ...
        const timeout = 2000;
        try {
            const request = new promise_1.promise({ url, timeout });
            const response = await request.execute();
        }
        catch (error) {
    ...
    it('should allow for debugging', async (done) => {
        ...
        try {
            ...
            const request = new promise_1.promise({ method: 'POST', url, body, debug: true });
            const response = await request.execute();
